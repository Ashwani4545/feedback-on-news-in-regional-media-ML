...
class TestAnalyzeText:
    ...
    def test_correction_flag_detected(self):
        from app.analysis import analyze_text
        result = analyze_text("Please correct the statistics in today's report, they are wrong.")
    ...
    def test_urgency_range(self):
        from app.analysis import analyze_text
        result = analyze_text("This is a shocking mistake, please correct the facts.")
    ...
    def test_indic_language_pass_through(self):
        from app.analysis import analyze_text
        # galat is in the transliterated CORRECTION_PATTERNS — test with transliterated Hindi
...
class TestUrgentEndpoint:
    def setup_method(self):
        ...
        client.post("/ingest_manual", json={
...
class TestCorrectionsEndpoint:
    ...
    def test_corrections_all_flagged(self):
        ...
        r = client.get("/corrections")
        # Just check the endpoint works — the filter is tested in analysis tests
        assert r.status_code == 200
...
class TestSentimentSummary:
    ...
    def test_percentages_sum_to_100_when_data_exists(self):
        # Ingest a mix
        client.post("/ingest_manual", json={"raw_text": "Good article!", "channel": "email"})
        client.post("/ingest_manual", json={"raw_text": "Wrong data, please correct.", "channel": "email"})
        r = client.get("/sentiment_summary", params={"days": 30})
        data = r.json()
        if data["total"] > 0:
            total_pct = data.get("pct_positive", 0) + data.get("pct_negative", 0)
...
class TestTrustEndpoint:
    def test_trust_returns_score(self):
        client.post("/ingest_manual", json={"raw_text": "Great reporting!", "channel": "email"})
...
class TestETLDeduplication:
    ...
    def test_etl_runs_analysis_on_tweets(self):
        from app.twitter_etl import insert_tweets
        from app.database import FeedbackProcessing
        ...
        db = TestSession()
        fb = db.query(__import__('app.database', fromlist=['Feedback']).Feedback)\
               .filter_by(tweet_id="test_tweet_analysis_001").first()
        assert fb is not None
        fp = db.query(FeedbackProcessing).filter_by(feedback_id=fb.feedback_id).first()
